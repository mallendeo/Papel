...
export const asyncFileReader = async files =>
  Promise.all(
    [...files].map(
      file =>
        new Promise((resolve, reject) => {
          ...
          reader.onload = e => {
            resolve(buffer.Buffer(e.target.result))
          }
          reader.onerror = e => {
            reject(new DOMException('Error trying to parse file'))
          }
...
export const generateHTML = (opts, extra = {}) => {
  ...
  const { code = {}, styles = [], scripts = [] } = opts
  ...
  return `
    ...
    <head>
      ...
      ${
        opts.ipfsDir
          ? `<base href="https://${IPFS_HOST}/ipfs/${opts.ipfsDir}">`
          : ''
      }
      ...
      ${styleArr.reduce(
        (str, href) => str + `<link rel="stylesheet" href="${href}">\n`,
        ''
      )}
    ...
    <body>
      ${code.html || ''}
      ${scriptArr.reduce(
        (str, src) => str + `<script src="${src}"></script>\n`,
        ''
      )}
      <!-- live -->
      ${
